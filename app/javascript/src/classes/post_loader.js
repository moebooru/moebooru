...
export default PostLoader = class PostLoader {
  constructor () {
    this.need_more_post_data = this.need_more_post_data.bind(this);
    this.hashchange_tags = this.hashchange_tags.bind(this);
    this.perform_search = this.perform_search.bind(this);
    this.xhr = new Set();
    document.on('viewer:need-more-thumbs', this.need_more_post_data);
    document.on('viewer:perform-search', this.perform_search);
  ...
  preload_sample_image () {
    let cached_sample_urls, post_id, sample_url;
  ...
  server_load_pool () {
    ...
    if (!this.result.disable_cache) {
      pool = this.cached_pools.get(this.result.pool_id);
      if (pool) {
    ...
    xhr = jQuery.ajax('/pool/show.json', {
      data: {
        id: this.result.pool_id
      },
      dataType: 'json'
    }).done((resp) => {
      this.result.pool = resp;
      return this.cached_pools.set(this.result.pool_id, this.result.pool);
    }).always(() => {
      this.xhr.delete(xhr);
      return this.request_finished();
    });
    this.xhr.add(xhr);
  }

  server_load_posts () {
    let results, search, tags, xhr;
    tags = this.result.tags;
    // Put holds:false at the beginning, so the search can override it.  Put limit: at
    // the end, so it can't.
    search = `holds:false ${tags} limit:${this.result.post_limit}`;
    if (!this.result.disable_cache) {
      results = this.cached_posts.get(search);
      if (results) {
    ...
    xhr = jQuery.ajax('/post.json', {
      ...
      return this.cached_posts.set(search, this.result.posts);
    }).fail(function (xhr, status) {
      let error, ref, ref1;
      error = (ref = (ref1 = resp.responseJSON) != null ? ref1.reason : void 0) != null ? ref : `error ${status}`;
      notice(`Error loading posts: ${error}`);
      return this.result.error = true;
    }).always(() => {
      this.xhr.delete(xhr);
      return this.request_finished();
    });
  ...
  request_finished () {
    let can_be_extended_further, i, new_post_ids, result;
    if (this.xhr.size > 0) {
      return;
    }
    ...
    document.fire('viewer:displayed-pool-changed', {
      pool: result.pool
    });
    document.fire('viewer:searched-tags-changed', {
      tags: result.tags
    });
  ...
  load (load_options) {
    let disable_cache, extending, limit, pool_id, tags;
    if (!load_options) {
      load_options = {};
    }
  ...
  perform_search (event) {
    let results_mode, tags;
    tags = event.memo.tags;
    this.last_seen_tags = tags;
    results_mode = event.memo.results_mode || 'center-on-first';

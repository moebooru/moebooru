...
window.time_ago_in_words = function(from_time, to_time)
{
...
window.start_notice_timer = function() {
...
Object.extend(Element.Methods, {
  ...
  show: function(element, visible) {
    if (visible || visible === null || visible === undefined)
...
window.OnKey = function(key, options, press, release)
{
  if(!options)
    options = {};
...
window.InitAdvancedEditing = function()
{
...
window.clone_event = function(orig)
{
  var e;
  if(document.dispatchEvent)
  {
...
window.createElement = function(type, className, html)
{
...
{
  try {
    var responses = this.transport.getAllResponseHeaders();
 ...
 * [5,4,6,3,7,2,8,1,9,0]
 */
window.sort_array_by_distance = function(list, idx)
{
  var ret = [];
  ret.push(list[idx]);
...
window.distance_squared = function(x1, y1, x2, y2)
{
...
window.create_canvas_2d = function()
{
 ...
 * scrolled. */
window.DragElement = function(element, options)
{
  $(document.body).addClassName("not-dragging");

  this.options = options || {};
  if(this.options.snap_pixels === null || this.options.snap_pixels === undefined)
...
DragElement.prototype.move_timer_update = function()
{
  ...
  if(this.last_event_params === null || this.last_event_params === undefined)
    return;
...
DragElement.prototype.handle_move_event = function(event, x, y)
{
  ...
  if(this.dragging_by_touch && Prototype.Browser.AndroidWebKit)
  {
    ...
    if(this.move_timer === null || this.move_timer === undefined)
...
DragElement.prototype.mousedown_event = function(event)
{
  ...
  if(this.ignore_mouse_events_until !== null && this.ignore_mouse_events_until !== undefined)
  {
...
{
  if(this.dragging_touch_identifier !== null && this.dragging_touch_identifier !== undefined)
    return;
 ...
 * element should be positioned (eg. position: absolute), and the element itself will be
 * scrolled. */
window.WindowDragElement = function(element)
{
  this.element = element;
 ...
 * scroll it around. */
window.WindowDragElementAbsolute = function(element, ondrag_callback)
{
  this.element = element;
...
window.FormatError = function(message, file, line, exc, info)
{
  var report = "";
...
window.ReportError = function(message, file, line, exc, info)
{
...
window.LocalStorageDisabled = function()
{
  if(!("localStorage" in window))
    return "unsupported";

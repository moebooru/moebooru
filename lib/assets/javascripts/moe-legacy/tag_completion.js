 ...
 */
window.TagCompletionClass = function()
{
...
TagCompletionClass.prototype.load_data = function(onComplete)
{
  ...
  if(this.loaded)
  {
    if(onComplete)
      onComplete();
    return this.tag_data !== null && this.tag_data !== undefined;
  }
  ...
  if(localStorage.tag_data !== null && localStorage.tag_data !== undefined)
  ...
  if(localStorage.tag_data !== null && localStorage.tag_data !== undefined)
  {
    if((this.most_recent_tag_data_version === null || this.most_recent_tag_data_version === undefined) || localStorage.tag_data_version === this.most_recent_tag_data_version)
    {
   ...
   * have. */
  var params = {};
  if(localStorage.tag_data_version !== null && localStorage.tag_data_version !== undefined)
...
TagCompletionClass.prototype.retrieve_tag_search = function(re, source, options)
{
  ...
  var max_results = 10;
  if(options.max_results !== null && options.max_results !== undefined)
...
{
  tags = tags.split(" ");
  if(old_tags !== null && old_tags !== undefined)
  ...
  tags.each(function(tag) {
     ...
     */
    if((old_tags === null || old_tags === undefined) && tag.indexOf(":") === -1)
    {
...
{
  if(this.tag_data === null || this.tag_data === undefined)
  ...
  results = results.slice(0, (options.max_results !== null && options.max_results !== undefined)? options.max_results:10);
...
window.TagCompletionBox = function(input_field)
{
...
TagCompletionBox.prototype.input_keydown = function(event)
{
  ...
  if(event.keyCode === Event.KEY_BACKSPACE)
  {
    ...
    if(this.rapid_backspaces_received > 1)
    {
      ...
      if(this.defer_timeout !== null && this.defer_timeout !== undefined)
...
TagCompletionBox.prototype.select_next = function(next)
{
  ...
  if(new_focus === null || new_focus === undefined)
...
TagCompletionBox.prototype.update = function(force)
{
   ...
   * completes. */
  if(TagCompletion.tag_data === null || TagCompletion.tag_data === undefined)
  {
...
if(TagCompletion === null || !("addEventListener" in document))
  window.TagCompletionBox = function() {};

...
export default User = class User {
  ...
  init () {
    let a, form, i, j, k, len, len1, len2, notice, ref, ref1, ref2;
  ...
  set_tab (tab) {
    let i, j, len, len1, li, passwordInput, ref, ref1, usernameInput;
  ...
  message (text) {
    let i, len, messageEl, messageId, ref;
    ref = this.messages;
    for (i = 0, len = ref.length; i < len; i++) {
      messageId = ref[i];
  ...
  form_username_changed (keyup) {
    ...
    checkName = () => {
      ...
      return this.check(username, null, true, (resp) => {
        let current_username, usernameInput;
        if (resp.exists) {
  ...
  form_submitted () {
    let email, password, password_confirm, username;
    this.cancel_check();
    window.clearTimeout(this.check_name_timer);
    username = document.getElementById('login-popup-username').value;

module AutoCompleteMacrosHelper      
  ...
  def auto_complete_field_with_style_and_script(object, method, tag_options = {}, completion_options = {})
    ...
    result << yield
    result << content_tag("div", "", :id => "#{object}_#{method}_auto_complete", :class => "auto_complete")
    result << auto_complete_field("#{object}_#{method}", { :url => { :action => "auto_complete_for_#{object}_#{method}" } }.update(completion_options))
    result.html_safe
  end

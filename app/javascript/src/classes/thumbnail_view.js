...
export default ThumbnailView = class ThumbnailView {
  ...
  loaded_posts_event (event) {
    let _frame, center_on_post_idx, frameIdx, i, initial_post_id, initial_post_id_and_frame, j, k, len, len1, old_centered_post_idx, old_post_ids, post, postId, post_id_search_order, post_id_to_search, post_ids, ref, results_mode;
  ...
  hashchange_post_id () {
    let new_post_idx, post_frame, post_id, post_id_and_frame;
  ...
  get_post_idx (post_id_and_frame) {
    let post_frame, post_frame_idx, post_id, post_idx;
    post_id = post_id_and_frame[0];
    post_frame = post_id_and_frame[1];
  ...
  show_next_post (prev) {
    let add, current_idx, new_idx, wrapped;
    if (this.post_ids.length === 0) {
      return;
    }
  ...
  add_post_to_display (right) {
    let node, post_idx_to_populate, thumb;
    node = this.container.down('.post-browser-posts');
    if (right) {
  ...
  get_width_adjacent_to_post (post_idx, right) {
    let center_post_right_edge, post, right_edge, rightmost_node;
    post = $('p' + post_idx);
    if (right) {
  ...
  center_on_post (post_idx) {
    let added, center_on_position, current_post_center, direction, distance, maximum_distance, minimum_distance, next_post, next_post_center, next_post_idx, node, pos, post, post_id, right, shift_pixels_right, thumb, width;
    if (!this.post_ids) {
  ...
  preload_thumbs () {
    let element, i, j, len, post, post_frame, post_id, post_idx, post_idxs, preload_post_idx, ref, url;
    post_idxs = [];
    i = 0;
    while (i < 5) {
  ...
  expand_post (post_idx) {
    let bottom, frame, image_url, image_width, left, max_width, overlay, post, post_frame, post_id, thumb;
  ...
  create_thumb (post_idx) {
    let div, img, item, post, post_frame, post_id, url;
    post_id = this.post_ids[post_idx];
    post_frame = this.post_frames[post_idx];
    post = Post.posts.get(post_id);
  ...
  set_thumb_dimensions (li) {
    let block_size, crop_left, frame, height, img, inner, pad_top, post, post_frame, post_id, post_idx, width;
  ...
  displayed_image_loaded_event (event) {
    let adjacent_post_idx, post_frame, post_id, post_ids_to_preload, post_idx;

...
export default RelatedTags = class RelatedTags {
  ...
  getTags () {
    let selectFrom, selectTo, selectionEnd, selectionStart, source, tags;
    source = this.source();
  ...
  highlightList () {
    let highlightedTags, i, j, len, len1, ref, ref1, results, t, tagLink;
    highlightedTags = {};
  ...
  buildList (title, tags) {
    let $tagLink, buf, i, len, ref, tag, tagName, tagsList;
  ...
  fetchSuccess (data) {
    let name, tag, tags, tagsCollection;
    tagsCollection = {};
    for (name in data) {
      tags = data[name];
  ...
  toggleTag (e) {
    let currentTags, jumpToEnd, newVal, tagName;
    e.preventDefault();
  ...
  run (e) {
    let data, doneCallback, source, tags, type, url;
    e.preventDefault();
    tags = this.getTags();

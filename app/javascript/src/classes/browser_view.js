...
export default BrowserView = class BrowserView {
  constructor(container) {
    ...
    this.container.down('.post-delete').on('click', (e) => {
      var default_reason, post, post_id, reason;
      e.stop();
  ...
  create_voting_popup() {
    var flash, last_dragged_over, popup_expand, popup_vote_widget_container;
    ...
    return this.popup_vote_dragger = new DragElement(popup_expand, {
      ...
      onup: (drag) => {
        var flash_x, flash_y, i, offset, star;
  ...
  preload(post_ids) {
    var frame, i, last_preload_request, new_preload_container, post, post_frame, post_id, ref;
  ...
  set_post(post_id, post_frame, lazy, no_hash_change, replace_history) {
    var is_cached, post, post_frame_hash;
    if (post_id == null) {
  ...
  edit_show(shown) {
    var post, rating_class, tags, tags_by_type;
    post = Post.posts.get(this.displayed_post_id);
    if (!post) {
      shown = false;
    }
  ...
  edit_post_area_changed() {
    var element, parent_id, post_edit, rating, source;
    post_edit = this.container.down('.post-edit');
  ...
  edit_save() {
    var edit_tags, post_id, rating_class, save_completed, selected_rating, tags;
    save_completed = () => {
      notice('Post saved');
  ...
  scale_and_position_image(resizing) {
    var img_box, original_height, original_width, post, ratio, window_size, x, y;
  ...
  update_navigator() {
    var height_percent, percent_x, percent_y, scroll_x, scroll_y, width_percent, x, y;
    if (!this.navigator) {
      return;
    }
    if (!this.img) {

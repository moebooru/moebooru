  ...
  module ClassMethods
    ...
    def calculate_related_by_type(tag, type, limit = 25)
      Rails.cache.fetch({ :category => :reltags_by_type, :type => type, :tag => tag }, :expires_in => 1.hour) do
        Tag.joins(:_posts).where(:posts => { :id => Post.has_tag(tag).select('posts.id') }, :tag_type => type).group(:name).count(:all, :order => 'count_all DESC', :limit => limit).reduce([]) do
    ...
    def calculate_related(tags, limit = 25)
      ...
      Rails.cache.fetch({ :category => :reltags, :tags => tags }, :expires_in => 1.hour) do
        Tag.joins(:_posts).where(:posts => { :id => Post.has_tags(tags, :only_ids => true) }).group(:name).count(:all, :order => 'count_all DESC', :limit => limit).reduce([]) do

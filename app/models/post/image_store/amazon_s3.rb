  ...
  module AmazonS3
    def move_file
      begin
        ...
        AWS::S3::S3Object.store(file_name, open(tempfile_path, "rb"), CONFIG["amazon_s3_bucket_name"], :access => :public_read, "Content-MD5" => base64_md5, "Cache-Control" => "max-age=315360000")

        if image?
          AWS::S3::S3Object.store("preview/#{md5}.jpg", open(tempfile_preview_path, "rb"), CONFIG["amazon_s3_bucket_name"], :access => :public_read, "Cache-Control" => "max-age=315360000")
        end

        if File.exist?(tempfile_sample_path)
          AWS::S3::S3Object.store("sample/" + CONFIG["sample_filename_prefix"] + "#{md5}.jpg", open(tempfile_sample_path, "rb"), CONFIG["amazon_s3_bucket_name"], :access => :public_read, "Cache-Control" => "max-age=315360000")
        end

        if File.exist?(tempfile_jpeg_path)
          AWS::S3::S3Object.store("jpeg/#{md5}.jpg", open(tempfile_jpeg_path, "rb"), CONFIG["amazon_s3_bucket_name"], :access => :public_read, "Cache-Control" => "max-age=315360000")
        end

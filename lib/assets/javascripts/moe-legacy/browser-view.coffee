...
window.BrowserView = (container) ->
  ...
  document.on 'viewer:vote', ((event) ->
    if @vote_widget
      @vote_widget.set event.memo.score
    return
  ...
  @container.down('.parent-post').down('A').on 'click', @parent_post_click_event.bindAsEventListener(this)
  @container.down('.child-posts').down('A').on 'click', @child_posts_click_event.bindAsEventListener(this)
  ...
  @edit_post_area_changed = @edit_post_area_changed.bind(this)
  post_edit.down('.edit-tags').on 'paste', ((e) ->
    @edit_post_area_changed.defer()
    return
  ).bindAsEventListener(this)
  post_edit.down('.edit-tags').on 'keydown', ((e) ->
    @edit_post_area_changed.defer()
    return
  ).bindAsEventListener(this)
  ...
  @frame_editor = new FrameEditor(@container.down('.frame-editor'), @img_box, @container.down('.frame-editor-popup'), onClose: (->
    @hide_frame_editor()
    return
  ).bind(this))
...
BrowserView::load_post_id_data = (post_id) ->
  debug 'load needed'
  ...
  new (Ajax.Request)('/post.json',
    ...
    method: 'get'
    onCreate: ((resp) ->
      @current_ajax_request = resp.request
      return
    ).bind(this)
    onSuccess: ((resp) ->
...
BrowserView::window_resize_event = (e) ->
  if e.stopped
    return
  @update_image_window_size()
  @scale_and_position_image true
...
BrowserView::scale_and_position_image = (resizing) ->
  img_box = @img_box
  if !@img
    return
...
BrowserView::center_image_on = (percent_x, percent_y) ->
...
BrowserView::select_edit_box = (className) ->
...
Navigator = (container, target) ->
  ...
  @container.show()
  @handlers = []
  @handlers.push @container.on('mousedown', @mousedown_event.bindAsEventListener(this))
  @handlers.push @container.on('mouseover', @mouseover_event.bindAsEventListener(this))
  @handlers.push @container.on('mouseout', @mouseout_event.bindAsEventListener(this))
  @dragger = new DragElement(@container,
    snap_pixels: 0
    onenddrag: @enddrag.bind(this)
    ondrag: @ondrag.bind(this))
  return

Navigator::set_image = (image_url, width, height) ->
  @img.src = image_url
  @img.width = width
  @img.height = height
  return

Navigator::enable = (enabled) ->
  @container.show enabled
  return

Navigator::mouseover_event = (e) ->
  if e.relatedTarget and e.relatedTarget.isParentNode(@container)
    return
...
Navigator::get_normalized_coords = (x, y) ->
...
Navigator::set_autohide = (autohide) ->
  @autohide = autohide
  @update_visibility()
  return

Navigator::update_visibility = ->
  box = @container.down('.image-navigator-box')
  visible = !@autohide or @hovering or @dragger.dragging

...
window.BrowserView = function(container)
{
...
*/
  ...
  this.container.down(".pool-info").on("click", ".remove-pool-from-post", function(e, element)
  {
    ...
    var pool = Pool.pools[pool_info.pool_id];
  ...
  document.on("posts:update", function(e) {
    if(e.memo.post_ids.get(this.displayed_post_id) === null || e.memo.post_ids.get(this.displayed_post_id) === undefined)
      return;
    this.set_post_info();
  }.bindAsEventListener(this));
   ...
   * dragging too. */
  if(this.image_swipe === null || this.image_swipe === undefined)
...
BrowserView.prototype.set_post_ui = function(visible)
{
   ...
   * shown. */
  this.container.down(".post-info").show(visible && this.displayed_post_id !== null && this.displayed_post_id !== undefined);
...
BrowserView.prototype.post_frame_list_includes = function(post_id_list, post_id, post_frame)
{
  ...
  return found_preload !== null && found_preload !== undefined;
}
...
BrowserView.prototype.load_post_id_data = function(post_id)
{
  ...
  // first to finish.
  if(this.current_ajax_request !== null && this.current_ajax_request !== undefined)
    return;
  ...
  new Ajax.Request("/post.json", {
    ...
    onComplete: function(resp) {
      ...
      if(!success && post_id === this.wanted_post_id)
      {
        ...
        if(this.displayed_post_id === null || this.displayed_post_id === undefined)
          document.fire("viewer:set-thumb-bar", {set: true});
...
BrowserView.prototype.set_viewing_larger_version = function(b)
{
  ...
  var post = Post.posts.get(this.displayed_post_id);
  var can_zoom = post !== null && post !== undefined && post.jpeg_url !== post.sample_url;
...
BrowserView.prototype.set_post = function(post_id, post_frame, lazy, no_hash_change, replace_history)
{
  ...
  if(post_frame === null || post_frame === undefined) {
...
BrowserView.prototype.set_post_info = function()
{
  ...
  if(post.creator_id !== null && post.creator_id !== undefined)
  {
  ...
  if(post.source !== "")
  {
    ...
    var m_old = post.source.match(/^http:\/\/.*pixiv\.net\/(img\d+\/)?img\/([-\w]+)\/(\d+)(_.+)?\.\w+$/)
    // pixiv URL without artist user id info (2014+)
    var m = post.source.match(/^https?:\/\/.*pixiv\.net\/img.*?(\d+)(_s|_m|(_big)?_p\d+)?\.\w+(\?\d+)?$/)
    if(m_old)
    {
    ...
    source_box.down("A").show(url !== null && url !== undefined);
    source_box.down("SPAN").show(url === null || url === undefined);
    if(url)
    {
  ...
  var has_image = post.file_url !== null && post.file_url !== undefined && !has_sample;
  ...
  parent_post.show(post.parent_id !== null && post.parent_id !== undefined);
  if(post.parent_id)
  ...
  {
    var byEl = flagged.down(".by");
    flagged.down(".flagged-by-box").show(post.flag_detail.user_id !== null && post.flag_detail.user_id !== undefined);
    if(post.flag_detail.user_id !== null && post.flag_detail.user_id !== undefined)
    {
  ...
  if(post.status === "deleted")
  {
    ...
    by_container.show(post.flag_detail.flagged_by !== null && post.flag_detail.flagged_by !== undefined);
...
BrowserView.prototype.update_navigator = function()
{
  ...
  var x = scroll_x + this.image_window_size.width/2;
  var y = scroll_y + this.image_window_size.height/2;
...
{
  if(this.lazy_load_timer === null || this.lazy_load_timer === undefined)
    return;
...
window.WindowTitleHandler = function()
{
  this.searched_tags = "";
  this.post_id = null;
...
WindowTitleHandler.prototype.update = function()
{
  ...
  if(this.pool)
  {
    ...
    {
      var pool_post = post.pool_posts[this.pool.id];
      if(pool_post)
      {
        var sequence = pool_post.sequence;
        title += " ";
...
BrowserView.prototype.parent_post_click_event = function(event)
{
  ...
  var post = Post.posts.get(this.displayed_post_id);
  if((post === null || post === undefined) || (post.parent_id === null || post.parent_id === undefined))
    return;
...
Navigator.prototype.ondrag = function(e)
{
  ...
  if(e.latest_event.shiftKey !== (this.shift_lock_anchor !== null && this.shift_lock_anchor !== undefined))
  {
...
Navigator.prototype.center_on_position = function(coords)
{
  ...
  {
    if(this.locked_to_x === null || this.locked_to_x === undefined)
    {

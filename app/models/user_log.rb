class UserLog < ActiveRecord::Base
  def self.access(user, request)
    ...
    return if !old_ip.nil? && old_ip == request.ip

    execute_sql("SELECT * FROM user_logs_touch(?, ?)", user.id, request.ip)

    # Clean up old records.
    execute_sql("DELETE FROM user_logs WHERE created_at < now() - interval '3 days'")

    Rails.cache.write("userip:#{user.id}", request.ip, :expires_in => 8.seconds)
  end
end

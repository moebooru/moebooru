this.PostUploadForm = function(form, progress)
{
  var XHRLevel2 = "XMLHttpRequest" in window && (new XMLHttpRequest().upload !== null && new XMLHttpRequest().upload !== undefined);
...
PostUploadForm.prototype.form_submit_event = function(e)
{
  ...
  if(this.current_request !== null && this.current_request !== undefined)
    return;
  ...
  if(post_file.files === null || post_file.files === undefined || post_file.files.length === 0)
    return;
 ...
 * results.
 */
this.UploadSimilarSearch = function(file_field, results)
{
...
UploadSimilarSearch.prototype.field_changed_event = function(event)
{
  ...
  if(this.file_field.files === null || this.file_field.files === undefined || this.file_field.files.length === 0)
    return;

...
export default Post = (function () {
  class Post {
    ...
    get_post_tags_with_type (post) {
      let results, tag_types, type_order, types;
    ...
    is_blacklisted (post_id) {
      let b, blacklist_applies, blacklists, has_tag, i, len, post;
      post = this.posts.get(post_id);
      if (post == null) {
      ...
      blacklist_applies = function (b) {
        let exclude, exclude_len, j, require, require_len;
        require = b.require;
    ...
    init_blacklisted (options) {
      let a, bl_entries, inactive_blacklists_hidden, li, list, sidebar, tags;
    ...
    resize_image () {
      let client_height, client_width, i, img, ratio;
      img = $('image');
    ...
    get_scroll_offset_to_center (element) {
      let left_spacing, offset, scroll_x, scroll_y, top_spacing, window_size;
    ...
    center_image (img) {
      let padding_left, padding_top, required_height, required_width, target_offset, window_size;
      if (!img) {
        img = $('image');
      }
      if (!img) {
    ...
    scale_and_fit_image (img) {
      let client_height, client_width, ratio, window_size;
      if (!img) {
        img = $('image');
      }
      if (!img) {
    ...
    hover_thumb_mouse_over (post_id, AlignItem, image, container) {
      let bottom_aligned_y, container_bottom, container_top, offset, post, y;
      post = this.posts.get(post_id);
      image.hide();
    ...
    hover_info_update () {
      let client_width, hover, hover_height, hover_thumb, hover_width, overlay, post, set_text_content, tags_by_type, thumb_center_x, thumb_offset, thumb_top_y, x, y;
    ...
    InitBrowserLinks () {
      let current, current_pool_id, parse_index_url, parse_url;
    ...
    cache_sample_urls () {
      let e, fifo, fifo_set, post_id, post_ids_to_expire, sample_urls;
    ...
    prompt_to_delete (post_id, completed) {
      let default_reason, flag_detail, reason;
      if (completed == null) {
        completed = function () {

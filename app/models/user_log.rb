class UserLog < ActiveRecord::Base
  def self.access(user, request)
    ...
    old_ip = Rails.cache.read("userip:#{user.id}") if CONFIG["enable_caching"]
    ...
    Rails.cache.write("userip:#{user.id}", request.remote_ip, :expires_in => 8.seconds) if CONFIG["enable_caching"]
  end
end

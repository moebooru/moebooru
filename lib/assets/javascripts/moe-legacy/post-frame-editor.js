...
var apply_drag = function(dragging_mode, x, y, image_dimensions, box)
{
  ...
  /* Apply the drag. */
  if(mode.top !== null && mode.top !== undefined)     result.top += y * mode.top;
  if(mode.left !== null && mode.left !== undefined)    result.left += x * mode.left;
  if(mode.right !== null && mode.right !== undefined)   right += x * mode.right;
  if(mode.bottom !== null && mode.bottom !== undefined)  bottom += y * mode.bottom;

  if(dragging_mode !== "move")
  {
    /* Only clamp the dimensions that were modified. */
    if(mode.left !== null && mode.left !== undefined)   result.left = clamp(result.left, 0, right-1);
    if(mode.top !== null && mode.top !== undefined)    result.top = clamp(result.top, 0, bottom-1);
    if(mode.bottom !== null && mode.bottom !== undefined) bottom = clamp(bottom, result.top+1, image_dimensions.height);
    if(mode.right !== null && mode.right !== undefined)  right = clamp(right, result.left+1, image_dimensions.width);
  }
...
window.FrameEditor = function(container, image_container, popup_container, options)
{
  ...
  for(var i = 0; i < popup_parts.length; ++i)
  {
    ...
    div = this.popup_container.down(part);
...
{
  var shown = (this.post_id !== null && this.post_id !== undefined) && (this.editing_frame !== null && this.editing_frame !== undefined) && this.show_corner_drag;
  ...
  this.dragger = new DragElement(this.main_frame, {
    ondown: function(e) {
      ...
      clicked_elements.each(function(e) {
        ...
        if((element === null || element === undefined) && e.hasClassName("frame-box-handle"))
          element = e;
      }.bind(this));
    ...
    ondrag: function(e) {
      ...
      if(this.dragging_new)
      {
        ...
        source_dims = frame_dimensions_from_image(dims, this.image_dimensions, post);
      ...
      dims = apply_drag(this.dragging_mode, e.aX, e.aY, this.image_dimensions, this.dragging_anchor);
...
FrameEditor.prototype.add_frame = function(new_frame)
{
  ...
  if(new_frame === null || new_frame === undefined)
    new_frame = {
...
FrameEditor.prototype.focus = function(post_frame)
{
  ...
  if(this.editing_frame !== null && this.editing_frame === undefined)
  {
    row = this.container.down(".frame-" + this.editing_frame);
...
window.CornerDragger = function(container, part, options)
{
  this.container = container;
  this.part = part;
  this.options = options;
...
{
  if((this.post_id === null || this.post_id === undefined) || (this.post_frame === null || this.post_frame === undefined))
    return;

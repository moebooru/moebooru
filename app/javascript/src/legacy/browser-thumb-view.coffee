...
window.ThumbnailView = (container, view) ->
  @container = container
  @view = view
  @post_ids = []
  @post_frames = []
...
ThumbnailView::window_resize_event = (e) ->
  if e.stopped
    return
...
ThumbnailView::loaded_posts_event = (event) ->
...
ThumbnailView::container_ondrag = (e) ->
  @centered_post_offset -= e.dX
  @center_on_post_for_scroll @centered_post_idx
  return

ThumbnailView::container_mouseover_event = (event) ->
  li = $(event.target)
...
ThumbnailView::container_mouseout_event = (event) ->
...
ThumbnailView::get_post_idx = (post_id_and_frame) ->
  post_id = post_id_and_frame[0]
  post_frame = post_id_and_frame[1]
...
ThumbnailView::container_mousemove_event = (e) ->
...
ThumbnailView::document_mouse_wheel_event = (event) ->
  event.stop()
  val = undefined
  if event.wheelDelta
...
ThumbnailView::set_active_post = (post_id_and_frame, lazy, center_thumbs, no_hash_change, replace_history) ->
...
ThumbnailView::show_next_post = (prev) ->
  if @post_ids.length == 0
    return
...
ThumbnailView::center_on_post_for_scroll = (post_idx) ->
  if @thumb_container_shown
    @expand_post null
...
ThumbnailView::remove_post = (right) ->
...
ThumbnailView::add_post_to_display = (right) ->
  node = @container.down('.post-browser-posts')
  if right
...
ThumbnailView::populate_post = (post_idx) ->
...
ThumbnailView::is_post_idx_shown = (post_idx) ->
  if post_idx >= @posts_populated[1]
    return false
...
ThumbnailView::get_width_adjacent_to_post = (post_idx, right) ->
  post = $('p' + post_idx)
  if right
...
ThumbnailView::center_on_post = (post_idx) ->
  if !@post_ids
...
ThumbnailView::expand_post = (post_idx) ->
...
ThumbnailView::create_thumb = (post_idx) ->
  post_id = @post_ids[post_idx]
  post_frame = @post_frames[post_idx]
  post = Post.posts.get(post_id)
...
ThumbnailView::set_thumb_dimensions = (li) ->
...
ThumbnailView::container_click_event = (event) ->
...
ThumbnailView::container_dblclick_event = (event) ->
  if event.button
    return
  event.preventDefault()
  @show_thumb_bar false
...
ThumbnailView::show_thumb_bar = (shown) ->
...
ThumbnailView::get_adjacent_post_idx_wrapped = (post_idx, next) ->
  post_idx += if next then +1 else -1
  post_idx = (post_idx + @post_ids.length) % @post_ids.length
  post_idx

ThumbnailView::displayed_image_loaded_event = (event) ->
...
InputHandler::handle_keypress = (e) ->
  key = e.charCode
  if !key
    key = e.keyCode
...
InputHandler::document_keypress_event = (e) ->

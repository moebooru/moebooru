  ...
  module ClassMethods
    def calculate_related_by_type(tag, type, limit = 25)
      if CONFIG["enable_caching"] && tag.size < 230
        results = Rails.cache.read("reltagsbytype/#{type}/#{tag}")
      ...
      if CONFIG["enable_caching"] && tag.size < 230
        ...
        Rails.cache.write("reltagsbytype/#{type}/#{tag}", results.map {|x| {"name" => x["name"], "post_count" => x["post_count"]}}.to_json, :expires_in => post_count.hours)
      end
